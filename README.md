...
Open [http://localhost:5000](http://localhost:5000). The layout is a single glass card that stacks on phones (camera upload via `capture`).

### Reverse image search keys

Copy `.env.example` to `.env` and set **one** of:

- `SERPAPI_API_KEY` — [SerpAPI](https://serpapi.com) Google reverse image (works from the uploaded file)
- `BING_VISUAL_SEARCH_KEY` — Azure Bing Visual Search (best native file upload)

Without a key, **text checks still work**. Image checks explain that a key is required.

Optional Kaggle ISOT files: put `True.csv` and `Fake.csv` in `data/` before `python scripts/train.py`. Otherwise the trainer downloads the public McIntire fake/real news CSV.

## API

- `POST /api/check-text` JSON `{ "text": "..." }`
- `POST /api/check-image` multipart `image` + optional `text`

## Project structure

```text
backend/     Flask app, NLP ensemble, reverse image search
frontend/     Responsive UI (college labels + image upload)
scripts/      Model training
models/       Saved joblib bundle after training
```
